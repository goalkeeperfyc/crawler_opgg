...
async def lst_page(lsturl, **kwarg):
    result_lst = []
    async with aiohttp.ClientSession() as sess_lst_page:
        get_lst_page = await sess_lst_page.get(lsturl, **kwarg)
        lst_page = await get_lst_page.text()
        video_lst = process_lst_page(resp=lst_page)
    ...
    for video_dic in video_lst:
        ...
        async with aiohttp.ClientSession() as sess_video_page:
            ...
            print("try to get %s video info" % url)
    ...
    dic_lst_to_file(listname=result_lst,
                    filename='/home/fangyucheng/python_code/output_result777.csv')
    return result_lst


def process_video_page(resp):
    page = resp
