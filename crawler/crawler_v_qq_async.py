...
def lst_page_task(page_num_max=34,
...
async def video_page(url, **kwarg):
    async with aiohttp.ClientSession() as session:
        get_page = await session.get(url, **kwarg)
        print('try to get %s data' % url)
        page = await get_page.text()
...
def process_video_page_response(response):
    if response is None:
        return None
    else:
...
tasks = [asyncio.ensure_future(request(url)) for url in lst_page_task_lst]
